...
export function ShowDetail() {
  ...
  if (libraryId == null || showKey == null) {
    return (
      <p className="auth-muted">
        <Link to="/" className="link-button">Back to library</Link>
      </p>
    )
  }
  ...
  if (error) {
    return (
      <p className="auth-muted">
        {error.message} · <Link to={`/library/${libraryId}`} className="link-button">Back</Link>
      </p>
    )
  }

  const posterUrl = series?.poster_path
    ? (series.poster_path.startsWith('http') ? series.poster_path : tmdbPosterUrl(series.poster_path, 'w500'))
    : episodes[0]?.poster_path
      ? tmdbPosterUrl(episodes[0].poster_path)
      : ''
  const backdropUrl = series?.backdrop_path
    ? (series.backdrop_path.startsWith('http') ? series.backdrop_path : tmdbBackdropUrl(series.backdrop_path, 'w780'))
      ...
      <div className="show-detail-header">
        ...
        <div className="show-detail-meta">
          ...
          {series?.first_air_date && (
            <p className="show-detail-date">{series.first_air_date}</p>
          )}
